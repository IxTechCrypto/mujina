...
def restart_miner():
    ...
    def run(script, timeout):
        with tempfile.TemporaryFile(mode="w+", encoding="utf-8",
            ...
            if is_windows:
                ...
                cmd = ["bash", os.path.join(HERE, script)]
    ...
    try:
        scripts = (
            ("stop-mujina.ps1", "start-mujina.ps1")
            if is_windows
            else ("stop-mujina.sh", "start-mujina.sh")
        )
        for script, label in zip(scripts, ("stop", "start")):
...
def read_logs(offset):
    ...
    try:
        ...
        with open(log_path, "r", encoding="utf-8", errors="replace") as f:
            if offset == 0:
                # Initial request: read the last 100 lines or last 15,000 bytes
                read_bytes = min(15000, file_size)
                if read_bytes > 0:
                    f.seek(file_size - read_bytes)
                    content = f.read()
                    # Split by newline and drop the first partial line
                ...
                if offset < file_size:
                    ...
                    # Log rolled over or was truncated; start from beginning
                    f.seek(0)
                    content = f.read()
                    lines = content.splitlines()
...
class H(http.server.BaseHTTPRequestHandler):
    ...
    def _send(self, code, body, ctype="application/json"):
        ...
        # Nothing here is ever cacheable. Reading dashboard.html dynamically
        # means UI changes take effect on browser refresh (F5) immediately.
    ...
    def do_GET(self):
        if self.path.startswith("/data"):
            ...
            try:
                new_offset, lines = read_logs(offset)
        ...
        elif self.path.startswith("/gradient.png") or self.path == "/gradient.png":
            img_path = os.path.join(HERE, "gradient.png")
